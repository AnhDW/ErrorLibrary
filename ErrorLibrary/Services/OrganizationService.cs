...
namespace ErrorLibrary.Services
{
    public class OrganizationService : IOrganizationService
    {
        ...
        public async Task<IEnumerable<object>> GetOrganizationTreeDropdown()
        {
            using var unitsContext = _dbContextFactory.CreateDbContext();
            using var factoriesContext = _dbContextFactory.CreateDbContext();
            using var enterprisesContext = _dbContextFactory.CreateDbContext();
            ...
            var tree = units.Select(u => new
            {
                ...
                text = u.Name,
                icon = "fa-solid fa-building",
                nodes = factoriesByUnit.TryGetValue(u.Id, out var factories) ?
                factories.Select(f => new
                {
                    id = $"factory_{f.Id}",
                    text = f.Name,
                    icon = "fa-solid fa-industry",
                    nodes = enterprisesByFactory.TryGetValue(f.Id, out var enterprises) ?
                    enterprises.Select(e => new
                    {
                        id = $"enterprise_{e.Id}",
                        text = e.Name,
                        icon = "fa-solid fa-store",
                        nodes = linesByEnterprise.TryGetValue(e.Id, out var lines) ?
                        lines.Select(l => new
                        {
                            id = $"line_{l.Id}",
                            text = l.Name,
